...
from ecdsa2 import ECDSA2_Params, ECDSA2
...
json_send({"command" : "get_challenge", "x" : int(public_key.x), "y" : int(public_key.y)})
challenge = json_recv()

# Sign the message
r, s = ecdsa.Sign(privkey=private_key, msg=challenge["msg"])

# Send the signature to the server for veriifcation
