...
# Compute x (k is constant)
k = ecdsa.Z_q(bits_to_int(hash_message_to_bits("Now you're just some value that I used to nonce"), ecdsa.q))
h = bits_to_int(hash_message_to_bits("Now you're just some value that I used to nonce"), ecdsa.q)

x = ecdsa.Z_q((signature["s"] * k - h**2) * inverse_mod((1337 * signature["r"]), ecdsa.q))
