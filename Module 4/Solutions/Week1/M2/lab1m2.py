...
from ecdsa2 import ECDSA2_Params, ECDSA2, hash_message_to_bits, bits_to_int
...
# Get the signature
while True:
    json_send({"command" : "get_signature", "msg" : "0"})
    signature1 = json_recv()
    r1 = signature1["r"]
    s1 = signature1["s"]
    json_send({"command" : "get_signature", "msg" : "1"})
    signature2 = json_recv()
    r2 = signature2["r"]
    s2 = signature2["s"]
...
h1 = bits_to_int(hash_message_to_bits("0"), ecdsa.q)
h2 = bits_to_int(hash_message_to_bits("1"), ecdsa.q)

assert r1 == r2

# Formula derived from the fundamental formula (slide 26)
k_inv = (s1 - s2 ) / (h1**2 - h2**2)
x = (s1 - k_inv * h1**2) / (k_inv * 1337 * r1)

# Sign the message
r, s = ecdsa.Sign(privkey=x, msg="gimme the flag")
